...
Для автоматического обновления используйте скрипт `update-production.sh`, который выполняет все необходимые шаги:

```bash
# Полное автоматическое обновление
./update-production.sh

# Обновление без создания резервной копии (не рекомендуется)
./update-production.sh --skip-backup

# Обновление без выполнения миграций
...
2. ✅ Остановку системы
3. ✅ Обновление кода из репозитория (git pull)
4. ✅ Пересборку Docker образов
5. ✅ Запуск системы
6. ✅ Выполнение миграций базы данных
7. ✅ Проверку статуса всех сервисов
...
### Регулярные задачи
```bash
# Обновление системы (еженедельно) - РЕКОМЕНДУЕТСЯ
./update-production.sh

# Или через старый скрипт (ручное обновление)
./deploy.sh update
